...
class QuizApp:
    ...
    def createWidgets(self):
        ...
        for i in range(numQuestions):
            ...
            checkQuestionButton = Button(
                self.mainFrame,
                text="Check",
                command=lambda index=i: self.checkQuestion(index)
            )
            checkQuestionButton.grid(
                row=i+1,
                column=2,
                padx=5,
                pady=5
            )
            self.questionWidgets.append(checkQuestionButton)
    ...
    def checkQuestion(self, i):
        index = i*4
        answer = self.questionWidgets[index+1].get()
        if self.quiz.checkAnswerAt(i, answer):
            self.questionWidgets[index+1].config(bg="green")
        else:
            self.questionWidgets[index+1].config(bg="red")
