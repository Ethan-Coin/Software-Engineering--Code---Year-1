...
class PosApp:
    ...
    def updateBill(self, totalLabel, newWin):
        value = self.newItemPrice.get()
        if not value:
            messagebox.showerror("Error", "Cannot add empty value")
            return
        elif self.newItemPrice.get() < 0:
            messagebox.showerror("Error", "Item price cannot be negative.")
            return
        else:
